...
# read in data
ratings <- fread("data/BX-Book-Ratings.csv", sep=";",nrows = 2000)
# ratings <- fread("data/ratings.csv")
# ratings$V1 <- NULL
names(ratings) <- c('user_id', 'ISBN', 'rating')
ratings[,book_id := .I]
# ratings <- head(ratings[rating != 0], 20000)
...
dimnames(ratingmat) <- list(book_id = ratings$book_id, 
